...
module "replica_log_bucket" {
  ...
  bucket = local.s3_replica_log_name
  ...
  policy        = data.aws_iam_policy_document.replica_log_bucket_policy.json
...
# Replica Log Bucket Policy
data "aws_iam_policy_document" "replica_log_bucket_policy" {
  statement {
    sid       = "S3ServerAccessLogsPolicy"
    effect    = "Allow"
    actions   = ["s3:PutObject"]
    resources = ["${module.replica_log_bucket.s3_bucket_arn}/log*"] ### DIRECT VALUE
    ...
    condition {
      ...
      values   = [module.replica_bucket.s3_bucket_arn] ### DIRECT VALUE
    }
